describe('model.Person', function() {
  ...
  beforeEach(module('eo.model'));
  beforeEach(module(function($provide) {
    visitor = {};
    $provide.value('model.visitor', visitor);
  }));
  beforeEach(inject(['model.Person', '$httpBackend', function(_Person_, _$httpBackend_) {
    Person = _Person_;
  ...
  describe('#create', function() {
    it('creates the person on the server', function() {
      $httpBackend
      .expectPOST('/people', { name: 'Ben' })
      .respond(200);

      var succeeded;
      new Person('Ben').create().then(function() {
        succeeded = true;
      });
      $httpBackend.flush();
      expect(succeeded).to.be.true; // jshint ignore:line
    });
  });
});
